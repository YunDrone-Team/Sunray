...
int UILogic::run() {
  try {
    return renderer_.run_with_build_callback([this]() { this->execute_build(); });
  } catch (const std::runtime_error &e) {
    if (std::string(e.what()) == "User requested exit") return 0;
...
std::vector<std::string> UILogic::get_selected_modules() const {
  return {state_.view.selected_modules.begin(), state_.view.selected_modules.end()};
}
...
std::string UILogic::get_project_root_dir() const {
  try {
...
#ifdef __APPLE__
    char exe_path_buf[PATH_MAX]; uint32_t size = sizeof(exe_path_buf);
    if (_NSGetExecutablePath(exe_path_buf, &size) == 0) {
      exe_path = std::filesystem::canonical(exe_path_buf);
    } else { throw std::runtime_error("无法获取可执行文件路径"); }
#else
    ...
    if (std::filesystem::exists(project_root / "build.sh")) return project_root.string();
    
    for (const auto &root_path : {"../../../", "../../", "../", "./"}) {
      std::filesystem::path test_path = std::filesystem::canonical(root_path);
      if (std::filesystem::exists(test_path / "build.sh")) return test_path.string();
    }
  } catch (const std::exception &e) {
...
void UILogic::execute_build() {
  ...
  std::string full_cmd = build_script + " --from-tui " + format_cli_arguments(selected_modules);
...
void UILogic::save_current_selection() const {
  ...
  try {
    ...
    if (!temp_file.is_open()) {
      std::cerr << "警告: 无法创建临时文件保存选择: " << temp_file_path << std::endl;
      return;
    }
