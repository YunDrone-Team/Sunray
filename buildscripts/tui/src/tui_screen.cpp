...
void ScreenCoordinateMapper::rebuild_dual_column_mapping(
    ...
    const std::vector<RenderItem> &right_items,
    int left_content_start_y,
    int right_content_start_y,
    int left_column_width,
    int right_column_start_x,
    int build_button_y, 
    bool dialog_shown,
    int dialog_ok_button_y,
    int right_scroll_offset,
    int right_visible_count) {
    ...
    for (size_t i = 0; i < left_items.size(); ++i) {
        ...
        ElementInfo element_info(ElementType::GROUP_HEADER, static_cast<int>(i), item.identifier);
    ...
    // 映射右栏项目（模块）- 考虑滚动偏移
    int visible_count = (right_visible_count > 0) ? right_visible_count : static_cast<int>(right_items.size());
    int end_index = std::min(static_cast<int>(right_items.size()), right_scroll_offset + visible_count);
...
ElementInfo ScreenCoordinateMapper::get_element_at(int screen_y, int screen_x) const {
    // 使用动态计算的布局参数，并确保考虑边框偏移
    const int left_column_max_x = left_column_width > 0 ? left_column_width : 32;  
    const int right_column_min_x = right_column_start_x > 0 ? right_column_start_x : 40;
    ...
    for (const auto& element : matching_elements) {
        // 对于特殊按钮，不需要X坐标过滤
        if (element.type == ElementType::BUILD_BUTTON || 
            element.type == ElementType::DIALOG_OK_BUTTON) {
            return element;
        }
