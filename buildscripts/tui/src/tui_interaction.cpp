...
InteractionManager::InteractionManager(const CoreData& core_data) : core_data_(core_data) {
    init_module_states();
}
...
bool InteractionManager::is_module_selected(const std::string& module_name) const {
    auto it = module_state_index_.find(module_name);
    return (it != module_state_index_.end()) ? module_states_[it->second].is_selected : false;
}

bool InteractionManager::is_module_disabled(const std::string& module_name) const {
    auto it = module_state_index_.find(module_name);
    return (it != module_state_index_.end()) ? module_states_[it->second].is_disabled : false;
}

void InteractionManager::toggle_module_selection(const std::string& module_name) {
    auto it = module_state_index_.find(module_name);
...
bool InteractionManager::toggle_group_selection(const std::string& group_name) {
    ...
    for (const std::string& module_name : group->modules) {
        ...
        if (is_selected) { any_selected = true; } else { all_selected = false; }
    }
    ...
    for (const std::string& module_name : group->modules) {
        ...
        if (it != module_state_index_.end()) {
            ModuleState& state = module_states_[it->second];
            if (target_state) {
                if (!state.is_disabled) { state.is_selected = true; }
            } else {
...
std::vector<std::string> InteractionManager::get_selected_modules() const {
    ...
    for (const auto& state : module_states_) {
        if (state.is_selected) { selected.push_back(state.name); }
    }
    return selected;
}
...
void InteractionManager::trigger_conflict_indication() {
    if (conflict_callback_) { conflict_callback_(); }
}
...
std::unordered_set<std::string> InteractionManager::get_selected_module_set() const {
    std::unordered_set<std::string> selected;
    for (const auto& state : module_states_) {
        if (state.is_selected) { selected.insert(state.name); }
    }
    return selected;
}
