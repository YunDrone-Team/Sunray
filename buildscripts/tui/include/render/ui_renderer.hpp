...
public:
    ...
    /**
     * 运行渲染器（带构建回调）
     * @param build_callback 构建按钮点击时的回调函数（已弃用，新架构中直接在TUI内处理）
     * @return 程序退出码
     */
    int run_with_build_callback(std::function<void()> build_callback = nullptr);
...
private:
     ...
     * @return 编译输出的FTXUI元素
     */
    ftxui::Element render_build_output();
