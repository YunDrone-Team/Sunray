...
uav_search_strategy::uav_search_strategy(ros::NodeHandle &nh_)
{
    ...
    // 挑战杯专用接口
    nh_.param<bool>("NeedBack", NeedBack, false);
    nh_.param<bool>("IsFindTargetLand", IsFindTargetLand, false);
    nh_.param<int>("YawDirection", YawDirection, -1);
    nh_.param<int>("rc_cmd", rc_cmd, 0);
...
{
    if((msg->channels[6] > 1000 || msg->channels[7] > 2000)) // 只有准备好之后，才接收通道值!!!!有问题，如果起飞失败就完了
    {
        uav_search_state.data = 1;
        uav_search_state_pub_.publish(uav_search_state); // 丢失
        std::cout << RED << "飞机已经切为手控!" << TAIL << std::endl;
        uav_state = -1; // 
    }

    if(msg->channels[5] > 1000 && msg->channels[5] < 2000 && uav_state!= -1 && rc_cmd == 1) // rc_cmd指令通道
    {
        rc_cmd = 2; // 解锁，起飞指令
        std::cout << GREEN << "收到遥控器解锁、起飞指令" << TAIL << std::endl;
    }
    if(msg->channels[5] > 2000 && uav_state!= -1 && rc_cmd == 2)
    {
        rc_cmd = 3; // 搜索指令
        std::cout << GREEN << "收到遥控器搜索指令" << TAIL << std::endl;
    }
    if(rc_cmd > 1 && msg->channels[5] < 1000 )
    {
        // 降落指令
        rc_cmd = -1;
    }
...
void uav_search_strategy::v_center_search_new()
{
    ...
    {
        if(NeedBack) // 判断是否需要返回
        { 
            std::cout << GREEN << "需要返回，开始返回..." << TAIL << std::endl;
            // 需要
            // 先判断是否有box可以跑
            if(!boxes_poses_.empty())
            {
        ...
        {
            std::cout << GREEN << "不需要返回，等待下一步指令..." << TAIL << std::endl;
            if(yolov8_detect_pose_.boxes[uav_group - 1].x == 0 && yolov8_detect_pose_.boxes[uav_group - 1].y == 0) // 为初始值
            {
                if(uav_search_state_all) // 其他无人机已经搜索完成，那么说明该机对应的靶标搜索失败，直接降落
                {
    ...
    else if(state == 3 && !land) // 遍历box
    {
        ...
        if(!target_poses_.empty())
        {
            if(is_to_target_pose(vins_pose, pos_))
            {
                ...
                {
                    if(IsFindTargetLand && 
                    !(yolov8_detect_pose_.boxes[uav_group - 1].x == 0 && yolov8_detect_pose_.boxes[uav_group - 1].y == 0)
                    ) // 找到就降落 并且当前靶标就是对应的组别，立即降落 
                    {
    ...
    else if(state == 5 && !land) // 调整机头旋转90度
    {
        ...
        {
            if(YawDirection == 1)
                cmd_.desired_yaw = min((vins_rpy[2] + YawDirection * M_PI / 24) , YawDirection * M_PI / 2);
            else if(YawDirection == -1)
                cmd_.desired_yaw = max((vins_rpy[2] + YawDirection * M_PI / 24) , YawDirection * M_PI / 2);
        }
        // std::cout << GREEN << "timer_cnt = " << timer_cnt << "s set angle is  " << cmd_.desired_yaw << TAIL << std::endl;
        ...
        if(fabs(vins_rpy[2] - (YawDirection * M_PI / 2)) < 0.01)
        {
            // std::cout << BLUE << "删除坐标：" << edges_poses_[edges_poses_.size() - 1].transpose() << TAIL << std::endl;
            // edges_poses_.erase(--edges_poses_.end());
            // pos_ = edges_poses_[edges_poses_.size() - 1];
        ...
        if(YawDirection == -1)
            std::cout << RED << "向右旋转...current_yaw: " << vins_rpy.transpose() << TAIL << std::endl;
        else
            std::cout << RED << "向左旋转...current_yaw: " << vins_rpy.transpose() << TAIL << std::endl;
    ...
    else if(!is_out_fence(vins_pose) && !land && rc_cmd != -1) // 在电子围栏内
    {
        if(state != 5)
        {
            // 速度控制
            if(velocity_ctrl_mode)
            {
                ...
                cmd_.enable_yawRate = false;
                cmd_.desired_yaw = 0;
                if(fabs(vins_rpy[2] - (YawDirection * M_PI / 2)) < 0.1) // 在横向搜索阶段
                {
                    cmd_.desired_yaw = YawDirection * M_PI / 2;
                    // std::cout << RED << "set angle is " << -M_PI / 2 << TAIL << std::endl;
                }
    ...
    if(rc_cmd == -1)
    {
        std::cout << RED << "遥控器降落...." << TAIL << std::endl;
        uav_search_state.data = 1; // 当前无人机搜索状态
        // 不能继续向前推进了，直接land，或者其他操作
        pos_[0] = vins_pose[0];
        pos_[1] = vins_pose[1];
...
void uav_search_strategy::ready_go()
{
    ...
    if(!uav_ready_go)
    {
        pos_ = Eigen::Vector3d(service_height, (2 - uav_id) * square_width, State1_height); // 注这里用于测试用，设置的2，因为实际测试的uav为1和3号
        ...
        if(rc_cmd == -1)
        {
            std::cout << RED << "遥控器降落..." << TAIL << std::endl;
            uav_search_state.data = 1; // 当前无人机搜索状态
            uav_search_state_pub_.publish(uav_search_state);
            cmd_.cmd = 3;
            cmd_pub_.publish(cmd_);
            mode_ = 0;
        }
        ...
        // 速度控制
        if(velocity_ctrl_mode && rc_cmd != -1)
        {
            double time = ros::Time::now().toSec();
            double dt = time - last_time;
            Eigen::Vector3d vel = vel_ctrl.calculate_velocity(vins_pose, pos_, dt, vel_from_autopilot);
...
void uav_search_strategy::pub_trajectory_cb(const ros::TimerEvent& e) 
{
    ...
    switch (mode_) {
        case 1: // takeoff
        {
            ...
            if(velocity_ctrl_mode && rc_cmd != -1) // 速度控制
            {
                // pos_ = Eigen::Vector3d(0,0,Takeoff_height); // 设置起飞点
                double time_ = ros::Time::now().toSec();
                double dt = time_ - last_time;
            ...
            if(is_to_target_pose(vins_pose, pos_))
            {
                ...
                if(yolo_detect_timer == 20)
                {
                    ...
                    if(takeoff || rc_cmd == 3) // 接收到起飞指令，直接进入搜索阶段，无需手动
                    {
                        mode_ = 3;
                        last_time = ros::Time::now().toSec(); // 记录上一次时间
...
{ 
    while (ros::ok()) {
        ...
        else if(!check_timeout())
        {
            if(rc_cmd == 2)
            {
                std::cout << GREEN << "开始解锁" << TAIL << std::endl;
                // 解锁
                setup_.cmd = 0;
                setup_.arming = 1;
                ...
                sleep(2); // 延迟2s后直接起飞
                std::cout << GREEN << "开始起飞" << TAIL << std::endl;
                mode_ = 1;
                last_time = ros::Time::now().toSec(); // 记录上一次时间
            }
            else if(rc_cmd == 0)
