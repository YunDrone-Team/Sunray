...
uav_search_strategy::uav_search_strategy(ros::NodeHandle &nh_)
{
    ...
    // 是否使用外部姿态估计
    nh_.param<bool>("use_external_attitude", use_external_attitude, false);
...
void uav_search_strategy::v_center_search_new()
{
    ...
    // }
    // 发布stage=1
    if(is_to_target_pose(vins_pose, pos_) && (state != 3 && uav_state == 1 && v_search_rounds == 0) && stage.data == 2)
    {
        stage.data = 1;
        stage_pub_.publish(stage); //纵向搜索，发送一个stage=1(开启搜索)的信号
    ...
    else if(state == 2 && !land) // 横向搜索
    {
        ...
        else
        {
            if(edges_poses_.empty())
            {
                ...
                edges_poses_.push_back(pos_);
                stage.data = 2;
                stage_pub_.publish(stage); //纵向搜索，发送一个stage=2(开启搜索)的信号
            }
            if(is_to_target_pose(vins_pose, pos_))
            {
                state = 1;
    ...
    {
        if(velocity_ctrl_mode && !use_external_attitude) // 速度控制
        {
            ...
            cmd_.cmd = sunray_msgs::UAVControlCMD::XYZ_VEL; // 速度控制
            cmd_.desired_vel[0] = vel[0];
            cmd_.desired_vel[1] = vel[1];
            cmd_.desired_vel[2] = vel[2];
            cmd_.enable_yawRate = false;
            // cmd_.desired_yaw = 0;
            cmd_.cmd_id = cmd_.cmd_id + 1;
            last_time = time_;

            std::cout << GREEN << "velocity_ctrl" << TAIL << std::endl;
        }
        else if(use_external_attitude && !velocity_ctrl_mode) // 姿态控制
        {
            cmd_.header.stamp = ros::Time::now();
            cmd_.cmd = sunray_msgs::UAVControlCMD::XYZ_ATT; // 姿态控制
            cmd_.desired_vel[0] = 0.0;
            cmd_.desired_vel[1] = 0.0;
            cmd_.desired_vel[2] = 0.0;
            cmd_.desired_pos[0] = pos_[0];
            cmd_.desired_pos[1] = pos_[1];
            cmd_.desired_pos[2] = pos_[2];
            cmd_.desired_yaw = 0;
            cmd_.desired_yaw_rate = 0;
            cmd_.enable_yawRate = false;
            cmd_.cmd_id = cmd_.cmd_id + 1;
            // cmd_pub_.publish(cmd_);
            std::cout << GREEN << "attitude_ctrl" << TAIL << std::endl;
        }
        else
        {
            cmd_.header.stamp = ros::Time::now();
            ...
            cmd_.cmd_id = cmd_.cmd_id + 1;
            // cmd_pub_.publish(cmd_);
            std::cout << GREEN << "pose_ctrl" << TAIL << std::endl;
        }
    ...
    { // targte_pose 为空，直接land
        ...
        pos_[1] = vins_pose[1];
        pos_[2] = 0.;
        cmd_.cmd = sunray_msgs::UAVControlCMD::Land;
        cmd_.cmd_id = cmd_.cmd_id + 1;
        cmd_pub_.publish(cmd_);
        mode_ = 0;
        land = 1;
    ...
    else if(!is_out_fence(vins_pose) && !land) // 在电子围栏内
    {
        ...
        {
            // 速度控制
            if(velocity_ctrl_mode  && !use_external_attitude)
            {
                double time = ros::Time::now().toSec();
                double dt = time - last_time;
                Eigen::Vector3d vel = vel_ctrl.calculate_velocity(vins_pose, pos_, dt, vel_from_autopilot);
...
void uav_search_strategy::ready_go()
{
    ...
    if(!uav_ready_go)
    {
        ...
        else if(use_external_attitude && !velocity_ctrl_mode) // 外部姿态控制
        {
            cmd_.header.stamp = ros::Time::now();
            cmd_.cmd = sunray_msgs::UAVControlCMD::XYZ_ATT; // 姿态控制
            cmd_.desired_vel[0] = 0.0;
    ...
    switch (mode_) {
        case 1: // takeoff
        {
            ...
            if(velocity_ctrl_mode && !use_external_attitude) // 速度控制
            {
                ...
                cmd_.desired_vel[1] = vel[1];
                cmd_.desired_vel[2] = vel[2];
                cmd_.desired_pos[0] = 0.0;
                cmd_.desired_pos[1] = 0.0;
                cmd_.desired_pos[2] = 0.0;
                // cmd_.desired_pos[2] = pos_[2];
                cmd_.enable_yawRate = false;
                cmd_.desired_yaw = 0;
                cmd_.cmd_id = cmd_.cmd_id + 1;
            ...
            else if(use_external_attitude && !velocity_ctrl_mode)
            {
                cmd_.header.stamp = ros::Time::now();
                cmd_.cmd = sunray_msgs::UAVControlCMD::XYZ_ATT; // 姿态控制
                cmd_.desired_vel[0] = 0.0;
        ...
        case 2: // move pose
        {
            ...
            else if(use_external_attitude && !velocity_ctrl_mode) // 使用外部姿态
            {
                cmd_.header.stamp = ros::Time::now();
                cmd_.cmd = sunray_msgs::UAVControlCMD::XYZ_ATT; // 姿态控制
                cmd_.desired_vel[0] = 0.0;
                    ...
                    else 
                    {
                        if (arming == 1){
                            ...
                            if(takeoff) // 如果有takeoff指令，则等待2s后直接起飞，无需手动
                            {
                                ...
                                last_time = ros::Time::now().toSec(); // 记录上一次时间
                                // 位置控制
                                cmd_.cmd = sunray_msgs::UAVControlCMD::Takeoff;
                                cmd_.cmd_id = cmd_.cmd_id + 1;
                                cmd_pub_.publish(cmd_);
...
{
    std::cout << GREEN << ">>>>>>>>>>>>>>>> UAV Search Indoor Param <<<<<<<<<<<<<<<<" << TAIL << std::endl;
