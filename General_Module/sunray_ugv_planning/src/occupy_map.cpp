...
void Occupy_map::init(ros::NodeHandle& nh)
{
    ...
    nh.param("ugv_planning/ugv_height", ugv_height, 0.2);
    ...
    // 地图实际尺寸，单位：米
    nh.param("map/map_size_x", map_size_3d_(0), 20.0);
    nh.param("map/map_size_y", map_size_3d_(1), 20.0);
    nh.param("map/map_size_z", map_size_3d_(2), 2.0);
    // localmap slide window
    nh.param("map/queue_size", queue_size, -1);
    // show border
    nh.param("map/border", show_border, true);
    // 地图分辨率，单位：米
    ...
    // 【发布】 地图rviz显示
    global_pcl_pub = nh.advertise<sensor_msgs::PointCloud2>(ugv_name + "/sunray/planning/global_pcl",  1); 
    // 【发布】 发布膨胀后的点云
    inflate_pcl_pub = nh.advertise<sensor_msgs::PointCloud2>(ugv_name + "/sunray/planning/global_inflate_pcl", 1);
...
// 地图更新函数 - 输入：viobot rdf点云
void Occupy_map::map_update_viobot(const sensor_msgs::PointCloud2ConstPtr & rdf_point, const nav_msgs::Odometry & odom)
{
    // 由sensor_msgs::PointCloud2 转为 pcl::PointCloud<pcl::PointXYZ>
    pcl::fromROSMsg(*rdf_point,*input_point_cloud);
