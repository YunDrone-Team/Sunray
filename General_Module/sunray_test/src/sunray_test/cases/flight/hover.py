...
class HoverCase(BaseCase):
    ...
    def execute(self, context, vehicle, event_logger):
        ...
        hover_reference = list(vehicle.state.position)
        if len(hover_reference) >= 3:
            hover_reference[2] = float(context.defaults["takeoff_target_z_m"])
        vehicle.hover(duration_s, target_z_m=float(context.defaults["takeoff_target_z_m"]))
        rospy.loginfo("[CASE] %s 完成", self.execution_context.case_id)
        return self._result(
            "pass",
            detail=f"hovered for {duration_s:.1f}s",
            metrics={"duration_s": duration_s},
            artifacts={"hover_reference_xyz": hover_reference},
        )
