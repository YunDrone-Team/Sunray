...
void target_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    Logger::print_color(int(LogColor::magenta), node_name, ": 收到新目标点: [" + 
...
SimpleObstacleInfo process_depth_image_simple()
{
    ...
    try {
        ...
        // 简单判断逻辑
        // 判断前方是否有障碍物
        if (center_dist > 0.1 && center_dist < safety_distance) {
            info.front_blocked = true;
        }
        
        // 判断左右是否畅通
        if (left_dist > 0.1 && left_dist < safety_distance) {
            info.left_clear = false;
        }
        if (right_dist > 0.1 && right_dist < safety_distance) {
...
int main(int argc, char **argv)
{
    ...
    nh.param<double>("flight_height", flight_height, 1.0);                   // 飞行高度，默认1米
    nh.param<double>("forward_vel", forward_vel, 0.5);                       // 前进速度，默认0.5m/s
    nh.param<double>("rotate_speed", rotate_speed, 0.3);                     // 旋转速度，默认0.3rad/s
    nh.param<double>("safety_distance", safety_distance, 1.5);               // 安全距离，默认1.5米
    ...
    Logger::print_color(int(LogColor::green), node_name, ": 初始目标位置: [" + 
                       std::to_string(target_pos.x) + ", " + 
                       std::to_string(target_pos.y) + ", " + 
                       std::to_string(target_pos.z) + "]");
