...
int obstacle_threshold = 1000; // 小于300mm的点视为障碍物
...
void target_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    Logger::print_color(int(LogColor::magenta), node_name, ": 收到新目标点: [" + std::to_string(target_pos.x) + ", " + std::to_string(target_pos.y) + ", " + std::to_string(target_pos.z) + "]");
}
