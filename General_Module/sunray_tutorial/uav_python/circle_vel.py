...
def sigint_handler(sig, frame):
    Logger.print_color(LogColor.green, node_name, '[circle_vel] exit...')
...
def main():
    ...
    #订阅无人机状态
    rospy.Subscriber(uav_name + '/sunray/uav_state', UAVState, uav_state_cb)
    #发布控制指令       
    ...
    # 设置无人机控制模式为CMD_CONTROL（同时，FMT会自动切换到OFFBOARD模式）
    ...
    for i in range(num_points):
        ...
        while not rospy.is_shutdown():
            ...
            dz = target_z - uav_state.position[2]
            ...
            # 发布速度指令
            uav_cmd.cmd = UAVControlCMD.XyzVel
            uav_cmd.desired_vel = [vx, vy, vz]
            control_cmd_pub.publish(uav_cmd)

            # 检查是否到达目标点
            if abs(uav_state.position[0] - target_x) < 0.15 and abs(uav_state.position[1] - target_y) < 0.15:
                break
            rate.sleep()
