#ifndef GROUND_MSG
#define GROUND_MSG

#include <cstdint>
#include <string.h>
#include <vector>
#include <string>
#include <iostream>
/*
    move mode
*/
#define CONTROL_TYPE_XYZ_POS 1
#define CONTROL_TYPE_XYZ_VEL 2
#define CONTROL_TYPE_XY_VEL_Z_POS 3
#define CONTROL_TYPE_XYZ_ATT 4
#define CONTROL_TYPE_TRAJECTORY 5

/*
    fly mode
*/
#define MODE_TYPE_OFFBOARD 1
#define MODE_TYPE_POSITION 2
#define MODE_TYPE_HOLD 3
#define MODE_TYPE_STABILIZED 4
#define MODE_TYPE_MANUAL 5

/*
    Vehicle control type
*/
#define VEHICLE_DISARM 0
#define VEHICLE_ARM 1
#define VEHICLE_TAKEOFF 2
#define VEHICLE_LAND 3
#define VEHICLE_HOVER 4
#define VEHICLE_KILL 5

/*
    MESSAGE ID
*/
#define HEARTBEAT_MESSAGE 1
#define STATE_MESSAGE 2
#define TAKEOFF_MESSAGE 101
#define CONTROL_MESSAGE 102
#define MODE_MESSAGE 103
#define VEHICLE_MESSAGE 104

/*
    size of struct
*/
#define SIZE_HEARTBEAT 8
#define SIZE_TAKEOFF 15
#define SIZE_MODE 15
#define SIZE_CONTROL 39
#define SIZE_STATUS 63

//
struct takeoff_msg
{
    uint32_t time_stamp; /**< @param time_stamp timestamp */
    uint8_t takeoff;     /**< @param takeoff takeoff flag */
};

struct mode_msg
{
    uint32_t time_stamp; /**< @param time_stamp timestamp */
    uint8_t uav_mode;    /**< @param uav_mode UAV mode */
};

struct vehicle_msg
{
    uint32_t time_stamp; /**< @param time_stamp timestamp */
    uint8_t type;        /**< @param type vehicle control type */
};

struct control_msg
{
    uint32_t time_stamp; /**< @param time_stamp timestamp */
    uint8_t type;        /**< @param type control type */
    float x;             /**< @param x x-coordinate */
    float y;             /**< @param y y-coordinate */
    float z;             /**< @param z z-coordinate */
    float yaw;           /**< @param yaw yaw angle */
    float roll;          /**< @param roll roll angle */
    float pitch;         /**< @param pitch pitch angle */
    bool yaw_rate;       /**< @param yaw_rate yaw rate flag */
    uint8_t frame;       /**< @param frame reference frame */
};

struct state_msg
{
    uint32_t time_stamp; /**< @param time_stamp timestamp */
    uint8_t uav_id;   /**< @param uav_id UAV ID */
    bool connected;   /**< @param connected connection status */
    bool armed;       /**< @param armed arm status */
    std::string mode; /**< @param mode flight mode lenght 15*/

    uint8_t location_source;          // 0: Mocap, 1: RTK, 2: Gazebo, 3:
    bool odom_valid;                  // 0: invalid, 1: valid
    std::vector<float> position;      // [m]
    std::vector<float> velocity;      // [m/s]
    std::vector<float> attitude;      // [rad]
    std::vector<float> attitude_rate; // [rad/s]
    std::vector<float> pos_setpoint;      // [m]
    std::vector<float> vel_setpoint;      // [m/s]
    std::vector<float> att_setpoint;      // [rad]

    float battery_state;      // [V]
    float battery_percentage; // [0-1]

    uint8_t control_mode;
};

// Message structures
struct Heartbeat_Message
{
    int32_t count;       /**< @param head message header */
    uint32_t time_stamp; /**< @param head message header */
};

// Message structures
struct Takeoff_Message
{
    uint16_t head;       /**< @param head message header */
    uint32_t length;     /**< @param length message length */
    uint8_t msg_id;      /**< @param msg_id message ID */
    uint8_t robot_id;    /**< @param robot_id robot ID */
    uint16_t check;      /**< @param check checksum */
    takeoff_msg payload; /**< @param payload takeoff message payload */
};

struct Mode_Message
{
    uint16_t head;    /**< @param head message header */
    uint32_t length;  /**< @param length message length */
    uint8_t msg_id;   /**< @param msg_id message ID */
    uint8_t robot_id; /**< @param robot_id robot ID */
    uint16_t check;   /**< @param check checksum */
    mode_msg payload; /**< @param payload mode message payload */
};

// Vehicle_Message
struct Vehicle_Message
{
    uint16_t head;       /**< @param head message header */
    uint32_t length;     /**< @param length message length */
    uint8_t msg_id;      /**< @param msg_id message ID */
    uint8_t robot_id;    /**< @param robot_id robot ID */
    uint16_t check;      /**< @param check checksum */
    vehicle_msg payload; /**< @param payload vehicle message payload */
};

struct Control_Message
{
    uint16_t head;       /**< @param head message header */
    uint32_t length;     /**< @param length message length */
    uint8_t msg_id;      /**< @param msg_id message ID */
    uint8_t robot_id;    /**< @param robot_id robot ID */
    uint16_t check;      /**< @param check checksum */
    control_msg payload; /**< @param payload control message payload */
};

struct State_Message
{
    uint16_t head;     /**< @param head message header */
    uint32_t length;   /**< @param length message length */
    uint8_t msg_id;    /**< @param msg_id message ID */
    uint8_t robot_id;  /**< @param robot_id robot ID */
    uint16_t check;    /**< @param check checksum */
    state_msg payload; /**< @param payload state message payload */
};

char *pack_Heartbeat_Message(int32_t count, uint32_t time_stamp)
{
    static char buf[sizeof(Heartbeat_Message)];
    memcpy(buf, &count, 4);
    memcpy(buf + 4, &time_stamp, 4);
    return buf;
}

char *encode_Heartbeat_Message(const Heartbeat_Message *msg)
{
    static char buf[sizeof(Heartbeat_Message)];
    memcpy(buf, &msg->count, 4);
    memcpy(buf + 4, &msg->time_stamp, 4);
    return buf;
}

char *pack_Takeoff_Message(uint16_t head, uint32_t length, uint8_t msg_id, uint8_t robot_id, uint16_t check, takeoff_msg payload)
{
    static char buf[sizeof(Takeoff_Message)];
    memcpy(buf, &head, 2);
    memcpy(buf + 2, &length, 4);
    memcpy(buf + 6, &msg_id, 1);
    memcpy(buf + 7, &robot_id, 1);
    memcpy(buf + 8, &check, 2);
    memcpy(buf + 10, &payload.time_stamp, 4);
    memcpy(buf + 14, &payload.takeoff, 1);
    return buf;
}

char *encode_Takeoff_Message(const Takeoff_Message *msg)
{
    static char buf[sizeof(Takeoff_Message)];
    memcpy(buf, &msg->head, 2);
    memcpy(buf + 2, &msg->length, 4);
    memcpy(buf + 6, &msg->msg_id, 1);
    memcpy(buf + 7, &msg->robot_id, 1);
    memcpy(buf + 8, &msg->check, 2);
    memcpy(buf + 10, &msg->payload.time_stamp, 4);
    memcpy(buf + 14, &msg->payload.takeoff, 1);
    return buf;
}

char *pack_Mode_Message(uint16_t head, uint32_t length, uint8_t msg_id, uint8_t robot_id, uint16_t check, mode_msg payload)
{
    static char buf[sizeof(Mode_Message)];
    memcpy(buf, &head, 2);
    memcpy(buf + 2, &length, 4);
    memcpy(buf + 6, &msg_id, 1);
    memcpy(buf + 7, &robot_id, 1);
    memcpy(buf + 8, &check, 2);
    memcpy(buf + 10, &payload.time_stamp, 4);
    memcpy(buf + 14, &payload.uav_mode, 1);
    return buf;
}

char *encode_Mode_Message(const Mode_Message *msg)
{
    static char buf[sizeof(Mode_Message)];
    memcpy(buf, &msg->head, 2);
    memcpy(buf + 2, &msg->length, 4);
    memcpy(buf + 6, &msg->msg_id, 1);
    memcpy(buf + 7, &msg->robot_id, 1);
    memcpy(buf + 8, &msg->check, 2);
    memcpy(buf + 10, &msg->payload.time_stamp, 4);
    memcpy(buf + 14, &msg->payload.uav_mode, 1);
    return buf;
}

char *pack_Vehicle_Message(uint16_t head, uint32_t length, uint8_t msg_id, uint8_t robot_id, uint16_t check, vehicle_msg payload)
{
    static char buf[sizeof(Vehicle_Message)];
    memcpy(buf, &head, 2);
    memcpy(buf + 2, &length, 4);
    memcpy(buf + 6, &msg_id, 1);
    memcpy(buf + 7, &robot_id, 1);
    memcpy(buf + 8, &check, 2);
    memcpy(buf + 10, &payload.time_stamp, 4);
    memcpy(buf + 14, &payload.type, 1);
    return buf;
}

char *encode_Vehicle_Message(const Vehicle_Message *msg)
{
    static char buf[sizeof(Vehicle_Message)];
    memcpy(buf, &msg->head, 2);
    memcpy(buf + 2, &msg->length, 4);
    memcpy(buf + 6, &msg->msg_id, 1);
    memcpy(buf + 7, &msg->robot_id, 1);
    memcpy(buf + 8, &msg->check, 2);
    memcpy(buf + 10, &msg->payload.time_stamp, 4);
    memcpy(buf + 14, &msg->payload.type, 1);
    return buf;
}

char *pack_Control_Message(uint16_t head, uint32_t length, uint8_t msg_id, uint8_t robot_id, uint16_t check, control_msg payload)
{
    static char buf[sizeof(Control_Message)];
    memcpy(buf, &head, 2);
    memcpy(buf + 2, &length, 4);
    memcpy(buf + 6, &msg_id, 1);
    memcpy(buf + 7, &robot_id, 1);
    memcpy(buf + 8, &check, 2);
    memcpy(buf + 10, &payload.time_stamp, 4);
    memcpy(buf + 14, &payload.type, 1);
    memcpy(buf + 15, &payload.x, 4);
    memcpy(buf + 19, &payload.y, 4);
    memcpy(buf + 23, &payload.z, 4);
    memcpy(buf + 27, &payload.yaw, 4);
    memcpy(buf + 31, &payload.roll, 4);
    memcpy(buf + 35, &payload.pitch, 4);
    memcpy(buf + 39, &payload.yaw_rate, 1);
    memcpy(buf + 40, &payload.frame, 1);
    return buf;
}

char *encode_Control_Message(const Control_Message *msg)
{
    static char buf[sizeof(Control_Message)];
    memcpy(buf, &msg->head, 2);
    memcpy(buf + 2, &msg->length, 4);
    memcpy(buf + 6, &msg->msg_id, 1);
    memcpy(buf + 7, &msg->robot_id, 1);
    memcpy(buf + 8, &msg->check, 2);
    memcpy(buf + 10, &msg->payload.time_stamp, 4);
    memcpy(buf + 14, &msg->payload.type, 1);
    memcpy(buf + 15, &msg->payload.x, 4);
    memcpy(buf + 19, &msg->payload.y, 4);
    memcpy(buf + 23, &msg->payload.z, 4);
    memcpy(buf + 27, &msg->payload.yaw, 4);
    memcpy(buf + 31, &msg->payload.roll, 4);
    memcpy(buf + 35, &msg->payload.pitch, 4);
    memcpy(buf + 39, &msg->payload.yaw_rate, 1);
    memcpy(buf + 40, &msg->payload.frame, 1);
    return buf;
}

char *pack_State_Message(uint16_t head, uint32_t length, uint8_t msg_id, uint8_t robot_id, uint16_t check, state_msg payload)
{
    static char buf[sizeof(State_Message)];
    memcpy(buf, &head, 2);
    memcpy(buf + 2, &length, 4);
    memcpy(buf + 6, &msg_id, 1);
    memcpy(buf + 7, &robot_id, 1);
    memcpy(buf + 8, &check, 2);
    memcpy(buf + 10, &payload.time_stamp, 4);
    memcpy(buf + 14, &payload.uav_id, 1);
    memcpy(buf + 15, &payload.connected, 1);
    memcpy(buf + 16, &payload.armed, 1);
    memcpy(buf + 17, payload.mode.c_str(), 15); // 修改这里，将长度改为15
    memcpy(buf + 32, &payload.location_source, 1);
    memcpy(buf + 33, &payload.odom_valid, 1);
    memcpy(buf + 34, &payload.position[0], 4);
    memcpy(buf + 38, &payload.position[1], 4);
    memcpy(buf + 42, &payload.position[2], 4);
    memcpy(buf + 46, &payload.velocity[0], 4);
    memcpy(buf + 50, &payload.velocity[1], 4);
    memcpy(buf + 54, &payload.velocity[2], 4);
    memcpy(buf + 58, &payload.attitude[0], 4);
    memcpy(buf + 62, &payload.attitude[1], 4);
    memcpy(buf + 66, &payload.attitude[2], 4);
    memcpy(buf + 70, &payload.attitude_rate[0], 4);
    memcpy(buf + 74, &payload.attitude_rate[1], 4);
    memcpy(buf + 78, &payload.attitude_rate[2], 4);
    memcpy(buf + 82, &payload.pos_setpoint[0], 4);
    memcpy(buf + 86, &payload.pos_setpoint[1], 4);
    memcpy(buf + 90, &payload.pos_setpoint[2], 4);
    memcpy(buf + 94, &payload.vel_setpoint[0], 4);
    memcpy(buf + 98, &payload.vel_setpoint[1], 4);
    memcpy(buf + 102, &payload.vel_setpoint[2], 4);
    memcpy(buf + 106, &payload.att_setpoint[0], 4);
    memcpy(buf + 110, &payload.att_setpoint[1], 4);
    memcpy(buf + 114, &payload.att_setpoint[2], 4);
    memcpy(buf + 118, &payload.battery_state, 4);
    memcpy(buf + 122, &payload.battery_percentage, 4);
    memcpy(buf + 126, &payload.control_mode, 1);
    return buf;
}

char *encode_State_Message(const State_Message *msg)
{
    static char buf[sizeof(State_Message)];
    memcpy(buf, &msg->head, 2);
    memcpy(buf + 2, &msg->length, 4);
    memcpy(buf + 6, &msg->msg_id, 1);
    memcpy(buf + 7, &msg->robot_id, 1);
    memcpy(buf + 8, &msg->check, 2);
    memcpy(buf + 10, &msg->payload.time_stamp, 4);
    memcpy(buf + 14, &msg->payload.uav_id, 1);
    memcpy(buf + 15, &msg->payload.connected, 1);
    memcpy(buf + 16, &msg->payload.armed, 1);
    memcpy(buf + 17, msg->payload.mode.c_str(), 15); // 修改这里，将长度改为15
    memcpy(buf + 32, &msg->payload.location_source, 1);
    memcpy(buf + 33, &msg->payload.odom_valid, 1);
    memcpy(buf + 34, &msg->payload.position[0], 4);
    memcpy(buf + 38, &msg->payload.position[1], 4);
    memcpy(buf + 42, &msg->payload.position[2], 4);
    memcpy(buf + 46, &msg->payload.velocity[0], 4);
    memcpy(buf + 50, &msg->payload.velocity[1], 4);
    memcpy(buf + 54, &msg->payload.velocity[2], 4);
    memcpy(buf + 58, &msg->payload.attitude[0], 4);
    memcpy(buf + 62, &msg->payload.attitude[1], 4);
    memcpy(buf + 66, &msg->payload.attitude[2], 4);
    memcpy(buf + 70, &msg->payload.attitude_rate[0], 4);
    memcpy(buf + 74, &msg->payload.attitude_rate[1], 4);
    memcpy(buf + 78, &msg->payload.attitude_rate[2], 4);
    memcpy(buf + 82, &msg->payload.pos_setpoint[0], 4);
    memcpy(buf + 86, &msg->payload.pos_setpoint[1], 4);
    memcpy(buf + 90, &msg->payload.pos_setpoint[2], 4);
    memcpy(buf + 94, &msg->payload.vel_setpoint[0], 4);
    memcpy(buf + 98, &msg->payload.vel_setpoint[1], 4);
    memcpy(buf + 102, &msg->payload.vel_setpoint[2], 4);
    memcpy(buf + 106, &msg->payload.att_setpoint[0], 4);
    memcpy(buf + 110, &msg->payload.att_setpoint[1], 4);
    memcpy(buf + 114, &msg->payload.att_setpoint[2], 4);
    memcpy(buf + 118, &msg->payload.battery_state, 4);
    memcpy(buf + 122, &msg->payload.battery_percentage, 4);
    memcpy(buf + 126, &msg->payload.control_mode, 1);
    return buf;
}


void unpack_Heartbeat_Message(const char *buf, Heartbeat_Message *msg)
{
    memcpy(&msg->count, buf, 4);
    memcpy(&msg->time_stamp, buf + 4, 4);
}

void unpack_Takeoff_Message(const char *buf, Takeoff_Message *msg)
{
    memcpy(&msg->head, buf, 2);
    memcpy(&msg->length, buf + 2, 4);
    memcpy(&msg->msg_id, buf + 6, 1);
    memcpy(&msg->robot_id, buf + 7, 1);
    memcpy(&msg->check, buf + 8, 2);
    memcpy(&msg->payload.time_stamp, buf + 10, 4);
    memcpy(&msg->payload.takeoff, buf + 14, 1);
}

void unpack_Mode_Message(const char *buf, Mode_Message *msg)
{
    memcpy(&msg->head, buf, 2);
    memcpy(&msg->length, buf + 2, 4);
    memcpy(&msg->msg_id, buf + 6, 1);
    memcpy(&msg->robot_id, buf + 7, 1);
    memcpy(&msg->check, buf + 8, 2);
    memcpy(&msg->payload.time_stamp, buf + 10, 4);
    memcpy(&msg->payload.uav_mode, buf + 14, 1);
}

void unpack_Vehicle_Message(const char *buf, Vehicle_Message *msg)
{
    memcpy(&msg->head, buf, 2);
    memcpy(&msg->length, buf + 2, 4);
    memcpy(&msg->msg_id, buf + 6, 1);
    memcpy(&msg->robot_id, buf + 7, 1);
    memcpy(&msg->check, buf + 8, 2);
    memcpy(&msg->payload.time_stamp, buf + 10, 4);
    memcpy(&msg->payload.type, buf + 14, 1);
}

void unpack_Control_Message(const char *buf, Control_Message *msg)
{
    memcpy(&msg->head, buf, 2);
    memcpy(&msg->length, buf + 2, 4);
    memcpy(&msg->msg_id, buf + 6, 1);
    memcpy(&msg->robot_id, buf + 7, 1);
    memcpy(&msg->check, buf + 8, 2);
    memcpy(&msg->payload.time_stamp, buf + 10, 4);
    memcpy(&msg->payload.type, buf + 14, 1);
    memcpy(&msg->payload.x, buf + 15, 4);
    memcpy(&msg->payload.y, buf + 19, 4);
    memcpy(&msg->payload.z, buf + 23, 4);
    memcpy(&msg->payload.yaw, buf + 27, 4);
    memcpy(&msg->payload.roll, buf + 31, 4);
    memcpy(&msg->payload.pitch, buf + 35, 4);
    memcpy(&msg->payload.yaw_rate, buf + 39, 1);
    memcpy(&msg->payload.frame, buf + 40, 1);
}

void unpack_State_Message(const char* buf, State_Message* msg) {
    memcpy(&msg->head, buf, 2);
    memcpy(&msg->length, buf + 2, 4);
    memcpy(&msg->msg_id, buf + 6, 1);
    memcpy(&msg->robot_id, buf + 7, 1);
    memcpy(&msg->check, buf + 8, 2);
    memcpy(&msg->payload.time_stamp, buf + 10, 4);
    memcpy(&msg->payload.uav_id, buf + 14, 1);
    memcpy(&msg->payload.connected, buf + 15, 1);
    memcpy(&msg->payload.armed, buf + 16, 1);
    char mode[256];
    memcpy(mode, buf + 17, 15); // 修改这里，将长度改为15
    msg->payload.mode = std::string(mode);
    memcpy(&msg->payload.location_source, buf + 32, 1);
    memcpy(&msg->payload.odom_valid, buf + 33, 1);
    memcpy(&msg->payload.position[0], buf + 34, 4);
    memcpy(&msg->payload.position[1], buf + 38, 4);
    memcpy(&msg->payload.position[2], buf + 42, 4);
    memcpy(&msg->payload.velocity[0], buf + 46, 4);
    memcpy(&msg->payload.velocity[1], buf + 50, 4);
    memcpy(&msg->payload.velocity[2], buf + 54, 4);
    memcpy(&msg->payload.attitude[0], buf + 58, 4);
    memcpy(&msg->payload.attitude[1], buf + 62, 4);
    memcpy(&msg->payload.attitude[2], buf + 66, 4);
    memcpy(&msg->payload.attitude_rate[0], buf + 70, 4);
    memcpy(&msg->payload.attitude_rate[1], buf + 74, 4);
    memcpy(&msg->payload.attitude_rate[2], buf + 78, 4);
    memcpy(&msg->payload.pos_setpoint[0], buf + 82, 4);
    memcpy(&msg->payload.pos_setpoint[1], buf + 86, 4);
    memcpy(&msg->payload.pos_setpoint[2], buf + 90, 4);
    memcpy(&msg->payload.vel_setpoint[0], buf + 94, 4);
    memcpy(&msg->payload.vel_setpoint[1], buf + 98, 4);
    memcpy(&msg->payload.vel_setpoint[2], buf + 102, 4);
    memcpy(&msg->payload.att_setpoint[0], buf + 106, 4);
    memcpy(&msg->payload.att_setpoint[1], buf + 110, 4);
    memcpy(&msg->payload.att_setpoint[2], buf + 114, 4);
    memcpy(&msg->payload.battery_state, buf + 118, 4);
    memcpy(&msg->payload.battery_percentage, buf + 122, 4);
    memcpy(&msg->payload.control_mode, buf + 126, 1);
}


bool check_header_and_length(const char *buf, size_t length)
{
    if (length < 10)
    {
        return false;
    }
    uint16_t head = *(uint16_t *)buf;
    uint32_t length_field = *(uint32_t *)(buf + 2);
    return head == 0XADFF && length_field == length - 2;
}

//
bool check_message(const char *buf, size_t length, uint8_t expected_msg_id, uint8_t expected_robot_id)
{
    if (!check_header_and_length(buf, length))
    {
        return false;
    }
    uint8_t msg_id = buf[6];
    uint8_t robot_id = buf[7];
    if (msg_id != expected_msg_id || robot_id != expected_robot_id)
    {
        return false;
    }
    return true;
}

void extract_message_header(const char *buf, uint16_t *head, uint32_t *length, uint8_t *msg_id, uint8_t *robot_id, uint16_t *check)
{
    memcpy(head, buf, 2);
    memcpy(length, buf + 2, 4);
    memcpy(msg_id, buf + 6, 1);
    memcpy(robot_id, buf + 7, 1);
    memcpy(check, buf + 8, 2);
}

// Calculate checksum for Takeoff_Message
uint16_t calculate_checksum_Takeoff_Message(const Takeoff_Message *msg)
{
    uint32_t checksum = 0;
    checksum += msg->head;
    checksum += msg->length;
    checksum += msg->msg_id;
    checksum += msg->robot_id;
    checksum += msg->payload.time_stamp;
    checksum += msg->payload.takeoff;
    checksum %= (1 << 16); // Limit the checksum to 16 bits
    return (uint16_t)checksum;
}

// Calculate checksum for Mode_Message
uint16_t calculate_checksum_Mode_Message(const Mode_Message *msg)
{
    uint32_t checksum = 0;
    checksum += msg->head;
    checksum += msg->length;
    checksum += msg->msg_id;
    checksum += msg->robot_id;
    checksum += msg->payload.time_stamp;
    checksum += msg->payload.uav_mode;
    return (uint16_t)checksum;
}

// Calculate checksum for Vehicle_Message
uint16_t calculate_checksum_Vehicle_Message(const Vehicle_Message *msg)
{
    uint32_t checksum = 0;
    checksum += msg->head;
    checksum += msg->length;
    checksum += msg->msg_id;
    checksum += msg->robot_id;
    checksum += msg->payload.time_stamp;
    checksum += msg->payload.type;
    return (uint16_t)checksum;
}

// Calculate checksum for Control_Message
uint16_t calculate_checksum_Control_Message(const Control_Message *msg)
{
    uint32_t checksum = 0;
    checksum += msg->head;
    checksum += msg->length;
    checksum += msg->msg_id;
    checksum += msg->robot_id;
    checksum += msg->payload.time_stamp;
    checksum += msg->payload.type;
    checksum += *(uint32_t *)&msg->payload.x;
    checksum += *(uint32_t *)&msg->payload.y;
    checksum += *(uint32_t *)&msg->payload.z;
    checksum += *(uint32_t *)&msg->payload.yaw;
    checksum += *(uint32_t *)&msg->payload.roll;
    checksum += *(uint32_t *)&msg->payload.pitch;
    checksum += msg->payload.yaw_rate;
    checksum += msg->payload.frame;
    return (uint16_t)checksum;
}

// Calculate checksum for State_Message
uint16_t calculate_checksum_State_Message(const State_Message* msg) {
    uint32_t checksum = 0;
    checksum += msg->head;
    checksum += msg->length;
    checksum += msg->msg_id;
    checksum += msg->robot_id;
    checksum += msg->payload.time_stamp;
    checksum += msg->payload.uav_id;
    checksum += msg->payload.connected;
    checksum += msg->payload.armed;
    // checksum += *(uint32_t*)msg->payload.mode.c_str();   // 字符串不参加校验
    checksum += msg->payload.location_source;
    checksum += msg->payload.odom_valid;
    checksum += *(uint32_t*)&msg->payload.position[0];
    checksum += *(uint32_t*)&msg->payload.position[1];
    checksum += *(uint32_t*)&msg->payload.position[2];
    checksum += *(uint32_t*)&msg->payload.velocity[0];
    checksum += *(uint32_t*)&msg->payload.velocity[1];
    checksum += *(uint32_t*)&msg->payload.velocity[2];
    checksum += *(uint32_t*)&msg->payload.attitude[0];
    checksum += *(uint32_t*)&msg->payload.attitude[1];
    checksum += *(uint32_t*)&msg->payload.attitude[2];
    checksum += *(uint32_t*)&msg->payload.attitude_rate[0];
    checksum += *(uint32_t*)&msg->payload.attitude_rate[1];
    checksum += *(uint32_t*)&msg->payload.attitude_rate[2];
    checksum += *(uint32_t*)&msg->payload.pos_setpoint[0];
    checksum += *(uint32_t*)&msg->payload.pos_setpoint[1];
    checksum += *(uint32_t*)&msg->payload.pos_setpoint[2];
    checksum += *(uint32_t*)&msg->payload.vel_setpoint[0];
    checksum += *(uint32_t*)&msg->payload.vel_setpoint[1];
    checksum += *(uint32_t*)&msg->payload.vel_setpoint[2];
    checksum += *(uint32_t*)&msg->payload.att_setpoint[0];
    checksum += *(uint32_t*)&msg->payload.att_setpoint[1];
    checksum += *(uint32_t*)&msg->payload.att_setpoint[2];
    checksum += *(uint32_t*)&msg->payload.battery_state;
    checksum += *(uint32_t*)&msg->payload.battery_percentage;
    checksum += msg->payload.control_mode;
    return (uint16_t)checksum;
}

#endif