...
class QuadrotorControlNodelet : public nodelet::Nodelet
{
public:
  QuadrotorControlNodelet()
...
void
QuadrotorControlNodelet::publishSO3Command(void)
{
...
void
QuadrotorControlNodelet::position_cmd_callback(
  const quadrotor_msgs::PositionCommand::ConstPtr& cmd)
{
...
void
QuadrotorControlNodelet::odom_callback(const nav_msgs::Odometry::ConstPtr& odom)
{
...
void
QuadrotorControlNodelet::enable_motors_callback(const std_msgs::Bool::ConstPtr& msg)
{
  if (msg->data)
...
void
QuadrotorControlNodelet::corrections_callback(
...
void
QuadrotorControlNodelet::imu_callback(const sensor_msgs::Imu& imu)
{
...
void
QuadrotorControlNodelet::onInit(void)
{
  ...
  odom_sub_ = n.subscribe("odom", 10, &QuadrotorControlNodelet::odom_callback, this,
                          ros::TransportHints().tcpNoDelay());
  position_cmd_sub_ =
    n.subscribe("position_cmd", 10, &QuadrotorControlNodelet::position_cmd_callback,
                this, ros::TransportHints().tcpNoDelay());

  enable_motors_sub_ =
    n.subscribe("motors", 2, &QuadrotorControlNodelet::enable_motors_callback, this,
                ros::TransportHints().tcpNoDelay());
  corrections_sub_ =
    n.subscribe("corrections", 10, &QuadrotorControlNodelet::corrections_callback,
                this, ros::TransportHints().tcpNoDelay());

  imu_sub_ = n.subscribe("imu", 10, &QuadrotorControlNodelet::imu_callback, this,
                         ros::TransportHints().tcpNoDelay());
}

#include <pluginlib/class_list_macros.h>
//PLUGINLIB_DECLARE_CLASS(so3_control, QuadrotorControlNodelet, QuadrotorControlNodelet,
//                        nodelet::Nodelet);
PLUGINLIB_EXPORT_CLASS(QuadrotorControlNodelet, nodelet::Nodelet);
