...
namespace gazebo
{
    ...
    void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf)
    {
        ...
        // 如果frameName前面有 xx::则去掉
        std::vector<std::string> frameName_values;
        boost::split(frameName_values, frameName, boost::is_any_of("::"));
        if (frameName_values.size() > 1)
        {
            // 取最后一个
            frameName = frameName_values.back();
        }
        ...
        // robot_namespace = "";
        // std::vector<std::string> ns_values;
        // std::string scopedName = _parent->ScopedName();
        // ROS_INFO_STREAM("robot ScopedName: " << scopedName);
        // boost::split(ns_values, scopedName, boost::is_any_of("::"));
        // if (ns_values.size() > 1)
        // {
        //     robot_namespace = ns_values.back();
        // }
        // else
        // {

        // }
        // ROS_INFO_STREAM("robot ScopedName: " << robot_namespace);
        ...
        {
            std::string scoped_name = _parent->ParentName();
...
#pragma omp critical
    ...
    void LivoxPointsPlugin::PublishLivoxROSDriverCustomMsg(std::vector<std::pair<int, AviaRotateInfo>> &points_pair)
    {
        ...
        // 获取仿真时间戳
        gazebo::common::Time current_time = raySensor->LastUpdateTime();
