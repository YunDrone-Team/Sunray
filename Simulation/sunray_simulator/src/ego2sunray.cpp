...
public:
    Ego2Sunray(ros::NodeHandle& nh): nh_(nh)
    {
        ...
        nh_.param<std::string>("odom_pub_topic", odom_pub_topic_, "/sunray/uav_control_cmd");

        std::cout << "uav_id: " << uav_id_ << std::endl;
        std::cout << "cmd_sub_topic: " << cmd_sub_topic_ << std::endl;
        std::cout << "odom_pub_topic: " << odom_pub_topic_ << std::endl;
        // 订阅ego-planner消息
        ego_sub_ = nh_.subscribe(cmd_sub_topic_, 1, &Ego2Sunray::egoCallback, this);
        // 发布控制到sunray
        cmd_pub_ = nh_.advertise<sunray_msgs::UAVControlCMD>(odom_pub_topic_, 1);
    }
