...
# --tab -e 'bash -c "sleep 2.0; roslaunch sunray_tutorial auto_land_by_pose.launch uav_id:=1; exec bash"' \
# --tab -e 'bash -c "sleep 2.0; roslaunch sunray_tutorial landmark_detection.launch uav_id:=1; exec bash"' \



#!/bin/bash
# start gazebo
gnome-terminal --window -e 'bash -c "roslaunch sunray_simulator sunray_sim_arcuo.launch; exec bash"' \
