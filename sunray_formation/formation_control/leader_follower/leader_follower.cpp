...
{
    if(mission_cmd == sunray_msgs::MissionCMD::TAKEOFF || mission_cmd == sunray_msgs::MissionCMD::LAND)
    {
        return;
    }
...
void LeaderFollower::formation_cmd_callback(const sunray_msgs::Formation::ConstPtr &msg)
{
...
// orca状态及控制指令回调
void LeaderFollower::orca_cmd_callback(const sunray_msgs::OrcaCmd::ConstPtr &msg)
{
    orca_cmd = *msg;
...
// 无人车状态回调
void LeaderFollower::ugv_state_cb(const sunray_msgs::UGVState::ConstPtr &msg, int i)
{
    agent_state[i].header.stamp = msg->header.stamp;
    agent_state[i].pose.pose.position.x = msg->position[0];
    agent_state[i].pose.pose.position.y = msg->position[1];
...
void LeaderFollower::uav_leader_callback(const sunray_msgs::UAVState::ConstPtr &msg)
{
...
void LeaderFollower::ugv_leader_callback(const sunray_msgs::UGVState::ConstPtr &msg)
{
    leader_pose_time = msg->header.stamp;
    leader_pose[0] = msg->position[0];
    leader_pose[1] = msg->position[1];
...
// 目标点回调
void LeaderFollower::goal_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    state = sunray_msgs::Formation::FORMATION;
    formation_type = sunray_msgs::Formation::GOAL;
    orca_setup.header.stamp = ros::Time::now();
...
// 动态编队
void LeaderFollower::dynamic_formation_pub(std::string name)
{
    /*
...
void LeaderFollower::dynamic_formation_online(std::string name)
{
...
void LeaderFollower::leader_formation_pub()
{
    // 如果自己是领队
...
// 根据ID计算跟随位置
std::tuple<float, float> LeaderFollower::calculateFollowerPosition(int vehicleId, double leaderPos_x, double leaderPos_y, double yaw,
...
void LeaderFollower::read_formation_yaml()
{
    // 检查文件是否存在
